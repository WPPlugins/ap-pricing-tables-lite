		...
		<div class='appts-tabs-content-wrapper'>
			...
			<div class='appts-tab-content appts-tab-table_name' style="display:block;">
				<div class="appts-input-wrap">
					...
					<div class="appts-info-wrap">
						<input type="text" name="name" value="<?php if(isset($table_data['name']) && $table_data['name'] !=''){ echo esc_attr($table_data['name']); } ?>">
				...
				<div class="appts-input-wrap">
					...
					<div class="appts-info-wrap">
						<input type="text" name="sid" value="<?php if(isset($table_data['sid']) && $table_data['sid'] !=''){ echo esc_attr($table_data['sid']); }else{ echo esc_attr($random_sid); } ?>">
				...
				if(isset($_GET['postid'])){ ?>
					<div class="appts-input-wrap">
						...
						<div class="appts-info-wrap">
							<input type="text" value="<?php if( isset( $table_data['sid'] ) && $table_data['sid'] !='' ){ echo esc_attr("[appts_pricing_table sid='{$table_data['sid']}']"); } ?>" readonly="readonly">
			...
			<div class='appts-tab-content appts-tab-styles' style="display:none;">
				<div class="appts-input-wrap">
					...
					<select name="template-selection" class="appts-img-selector">
						...
						<?php for($i=01; $i<=5; $i++){ $template_value ='template_'.sprintf("%02d", $i) ; ?>
							<option value='<?php echo $template_value; ?>' data-src="<?php echo APPTS_IMAGE_DIR . "/templates/template_".sprintf("%02d", $i).".jpg"; ?>" <?php if(isset($table_data['template-selection']) && $table_data['template-selection'] == "$template_value"){ echo "selected='selected'"; $style_img_src = APPTS_IMAGE_DIR . "/templates/".$template_value.".jpg";  } ?> ><?php echo "Template $i"; ?></option>
						<?php } ?>
					</select>
				...
				<div class="appts-checkbox-selection-wrap appts-checkbox-column-border-radius-settings">
					<div class="appts-input-wrap">
						...
						<div class='appts-info-wrap'>
							<input type="checkbox" class='appts-checkbox-option' id='appts-column-border-radius-enable' name="col-border" value="1" <?php if(isset($table_data['col-border'])){ checked( $table_data['col-border'], 1 ); } ?> >
							<div class="appts-info"><?php _e( 'Option to enable the border radius of the column.', 'ap-pricing-tables-lite' ); ?></div>
						</div>
					</div>
					<!-- If enabled -->
					<div class="appts-checkbox-options-show appts-column-border-radius-options" style='<?php if(isset($table_data['col-border'])){ echo 'display:block;'; }else{ echo "display:none;";  }  ?>'>
						<div class="appts-input-wrap appts-input-manage">
				            <label><?php _e( 'Column Border Radius', 'ap-pricing-tables-lite' ); ?></label>
				           	<div class="appts-info-wrap">
					            <input type="text" name="col-border-radius[top]" value="<?php if( isset($table_data['col-border-radius']['top']) && $table_data['col-border-radius']['top'] !='' ){ echo esc_attr($table_data['col-border-radius']['top']); } ?>" class="appts-input-mid">
					            <input type="text" name="col-border-radius[right]" value="<?php if( isset($table_data['col-border-radius']['right']) && $table_data['col-border-radius']['right'] !='' ){ echo esc_attr($table_data['col-border-radius']['right']); } ?>" class="appts-input-mid">
					            <input type="text" name="col-border-radius[bottom]" value="<?php if( isset($table_data['col-border-radius']['bottom']) && $table_data['col-border-radius']['bottom'] !='' ){ echo esc_attr($table_data['col-border-radius']['bottom']); } ?>" class="appts-input-mid">
					            <input type="text" name="col-border-radius[left]" value="<?php if( isset($table_data['col-border-radius']['left']) && $table_data['col-border-radius']['left'] !='' ){ echo esc_attr($table_data['col-border-radius']['left']); } ?>" class="appts-input-mid">
						...
						<div class="appts-input-wrap appts-input-manage">
							...
							<div class="appts-info-wrap">
								<input type="text" name="col-space" value="<?php if( isset($table_data['col-space']) && $table_data['col-space'] !='' ){ echo esc_attr($table_data['col-space']); } ?>">
						...
						<div class="appts-input-wrap appts-input-manage">
							...
							<div class="appts-info-wrap">
								<input type="text" name="col-width" value="<?php if( isset($table_data['col-width']) && $table_data['col-width'] !='' ){ echo esc_attr($table_data['col-width']); } ?>">
			...
			<div class='appts-tab-content appts-tab-currency' style="display:none;">
				<div class='appts-currency-selection appts-input-wrap'>
					...
					<div class="appts-info-wrap">
						<select name="currency[id]">
							...
							?>
							<option value="<?php echo $value['id']; ?>"<?php if(isset($table_data['currency']['id'])){ selected( $value['id'], $table_data['currency']['id'] ); } ?>><?php echo $value['name']."( {$value['symbol']} )"; ?></option>
							<?php
							endforeach;
							?>
						</select>
				...
				<div class='appts-currency-position appts-input-wrap'>
					...
					<div class="appts-info-wrap">
						<select name="currency[position]">
								<option value="left" 	<?php if(isset($table_data['currency']['position'])) { selected( 'left', $table_data['currency']['position'] ); } ?> 	><?php _e( 'Left (e.g. $100)', 'ap-pricing-tables-lite' ); ?></option>
								<option value="right" 	<?php if(isset($table_data['currency']['position'])) { selected( 'right', $table_data['currency']['position'] ); } ?> 	><?php _e( 'Right (e.g. 100$)', 'ap-pricing-tables-lite' ); ?></option>
						</select>
						<div class="appts-info">
				...
				<div class='appts-currency-t-separator appts-input-wrap'>
					...
					<div class="appts-info-wrap">
						<input type="text" name="currency[thousand_sep]" value="<?php if(isset( $table_data['currency']['thousand_sep'] ) && $table_data['currency']['thousand_sep'] != ''){ echo esc_attr($table_data['currency']['thousand_sep']); } ?>" />
				...
				<div class="appts-decimal-seperator appts-input-wrap">
					...
					<div class="appts-info-wrap">
						<input type="text" name="currency[decimal_sep]" value="<?php if(isset( $table_data['currency']['decimal_sep'] ) && $table_data['currency']['decimal_sep'] != '' ){ echo esc_attr($table_data['currency']['decimal_sep']); }else{ echo ".";  } ?>" />
				...
				<div class="appts-currency-no-of-dec appts-input-wrap">
					...
					<div class="appts-info-wrap">
						<input type="text" name="currency[decimal_no]" value="<?php if(isset( $table_data['currency']['decimal_no'] ) && $table_data['currency']['decimal_no'] != ''){ echo esc_attr($table_data['currency']['decimal_no']); } ?>" />
